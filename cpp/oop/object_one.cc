...
//=========================================== BEST PRACTICE ===========================================
//=====================================================================================================
//Implement the simplest relationship type that meets the needs of your program, not what seems right 
//in real-life. For example, if you’re writing a body shop simulator, you may want to implement a car 
//and engine as an aggregation, so the engine can be removed and put on a shelf somewhere for later. 
//However, if you’re writing a racing simulation, you may want to implement a car and an engine as a 
//composition, since the engine will never exist outside of the car in that context.
//=====================================================================================================

//In the Department/Teacher example above, we used a reference in the Department to store the Teacher. 
//This works fine if there is only one Teacher, but what if a Department has multiple Teachers? We’d 
//like to store those Teachers in a list of some kind (e.g. a std::vector) but fixed arrays and the 
//various standard library lists can’t hold references (because list elements must be assignable, and 
//references can’t be reassigned). Solution: std::reference_wrapper, lives in the <functional> header.
